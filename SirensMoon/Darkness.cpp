...
void Darkness::Update(Vector2 window_pos, Vector2 camera_pos) {
	...
	for (auto&& actor : _game.GetActorServer()->GetObjects()) {
		if (actor->GetType() == Actor::Type::Light1) {
			DrawGraph(actor->GetPosition().x+window_pos.x-240.0 - _splitWindow.GetCamera()->GetPosition().x,
				actor->GetPosition().y + window_pos.y -240.0 - _splitWindow.GetCamera()->GetPosition().y, _cg, 1);
		}
		if (actor->GetType() == Actor::Type::Light2) {
			DrawGraph(actor->GetPosition().x + window_pos.x - 240.0 - _splitWindow.GetCamera()->GetPosition().x,
				actor->GetPosition().y + window_pos.y - 240.0 - _splitWindow.GetCamera()->GetPosition().y, _cg2, 1);
		}
	}
}
